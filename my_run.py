...
def run_train(spk_id, size, learning_rate, model_name, partition='120sec', load_ckpt=False, max_iter=None):
    ...
    with wandb.init(config=config, project='pse'):  # , entity='jsbae'):
        ...
        train_total_num = batch_sampler.get_data_len(int(spk_id), 'train')
        print('# # of train samples:', train_total_num)
    
        if not IS_RAND_VAL:
            # Load validation data
            # for fixed val samples and noises
            val_total_num = val_batch_sampler.get_data_len(int(spk_id), 'val')  # repeat 5 times
            print('# # of val samples:', val_total_num)
            val_batch_1 = val_batch_sampler.sample_batch(int(spk_id), val_total_num, 'val')
            val_batch_2 = val_batch_sampler.sample_batch(int(spk_id), val_total_num, 'val')
            val_batch_3 = val_batch_sampler.sample_batch(int(spk_id), val_total_num, 'val')
            VAL_BATCH_SIZE = 8
