...
for spk in ${speakers[@]}; do
    for s in ${sizes[@]}; do
        for r in ${rates[@]}; do
            ...
            CUDA_VISIBLE_DEVICES=7 python my_run.py  -s $spk -r $r -i $s -p 'xttsv2_synth_50utt_virtual' -m 'xttsv2'
            CUDA_VISIBLE_DEVICES=7 python my_run.py  -s $spk -r $r -i $s -p 'xttsv2_synth_250utt_virtual' -m 'xttsv2'
        done
    done
done